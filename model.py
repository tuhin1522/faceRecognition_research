...
from tensorflow.keras.layers import Lambda, Flatten, Dense, GlobalAveragePooling2D
...
def create_model():
    ...
    #x = Lambda(lambda x: tf.nn.local_response_normalization(x), name='lrn_1')(x)
    ...
    #x = Lambda(lambda x: tf.nn.local_response_normalization(x), name='lrn_2')(x)
    x = ZeroPadding2D(padding=(1, 1))(x)
    x = MaxPooling2D(pool_size=3, strides=2)(x)
